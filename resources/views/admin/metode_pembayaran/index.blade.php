<x-admin-layout title="Metode Pembayaran Platform">
    <div class="p-4">
        <div class="flex items-end justify-between">
            ...
            <div>
                <p class="text-sm text-gray-400">Showing {{ $metodePembayaran->count() }} of {{ $metodePembayaran->count() }} records</p>
            </div>
        </div>
            ...
            <div class="overflow-x-auto rounded-lg">
                <table class="w-full text-sm text-left border-collapse montserrat-font">
                    <thead class="bg-white text-gray-500 shadow-md font-medium p-2">
                        <tr>
                            <td class="px-6 py-3 border-b">Jenis Metode</td>
                            <td class="px-6 py-3 border-b">Nama Metode</td>
                            <td class="px-6 py-3 border-b">Nomor Rekening</td>
                            <td class="px-6 py-3 border-b">Nama Pemilik</td>
                            <td class="px-6 py-3 border-b">Deskripsi</td>
                    ...
                    <tbody class="divide-y">
                        @forelse ($metodePembayaran as $metode)
                            <tr class="bg-white hover:bg-gray-50 transition">
                                <td class="px-6 py-4">{{ $metode->jenis_metode }}</td>
                                <td class="px-6 py-4">{{ $metode->nama_metode }}</td>
                                <td class="px-6 py-4">{{ $metode->nomor_rekening_platform }}</td>
                                <td class="px-6 py-4">{{ $metode->nama_pemilik_platform }}</td>
                                <td class="px-6 py-4">{{ $metode->deskripsi ?? '-' }}</td>
                                <td class="px-6 py-4">
                                    <div
                                        class="{{ $metode->is_active ? 'bg-green-600/20 text-green-600' : 'bg-red-600/20 text-red-600' }} p-2 text-center rounded-full font-bold">
                                        {{ $metode->is_active ? 'Active' : 'Inactive' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('admin.metodePembayaran.edit', $metode->id_metode_pembayaran_platform) }}"
                                        class="text-blue-600 hover:underline">Edit</a>
                                    <form action="{{ route('admin.metodePembayaran.destroy', $metode->id_metode_pembayaran_platform) }}" method="POST"
                                        class="inline"
