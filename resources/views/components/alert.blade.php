...
<div x-data="{
    ...
    init() {
        setTimeout(() => { 
            ...
            setTimeout(() => {
                this.show = false;
            }, 2500); // Menghilang setelah 2.5 detik
        }, 300); // Delay 300ms sebelum muncul
    }
}" x-init="show = false" x-show="show" x-transition:enter="transition ease-out duration-500 transform"
    x-transition:enter-start="opacity-0 scale-90 translate-x-full"
    x-transition:enter-end="opacity-100 scale-100 translate-x-0"
    x-transition:leave="transition ease-in duration-300 transform"
    x-transition:leave-start="opacity-100 scale-100 translate-x-0"
    x-transition:leave-end="opacity-0 scale-90 translate-x-full" style="display: none;"
    class="bg-white p-4 rounded-md shadow-md mt-4 mr-8 fixed top-0 right-0 z-40 will-change-transform">
    <div class="pr-6">
