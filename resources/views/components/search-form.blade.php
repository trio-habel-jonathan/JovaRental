...
<form {{ $attributes->merge(['action' => $action, 'method' => $method, 'data-aos' => 'zoom-in', 'data-aos-delay' => '150']) }} class="{{ $class }}">
    ...
    <div class="flex flex-row lg:flex-col items-start justify-center gap-4">
        <div class="driver-option">
            <input type="radio" id="tanpa_sopir" name="tipe_rental" value="tanpa_sopir" checked class="hidden peer" onchange="toggleDriverOptions()">
            <label for="tanpa_sopir" class="cursor-pointer w-48 text-nowrap flex items-center justify-center px-6 py-3 text-center border-2 border-gray-300 rounded-md peer-checked:border-primary peer-checked:bg-primary-50 peer-checked:text-darkprimary hover:bg-gray-50 font-medium">
                Tanpa Sopir
            </label>
        </div>
        <div class="driver-option">
            <input type="radio" id="dengan_sopir" name="tipe_rental" value="dengan_sopir" class="hidden peer" onchange="toggleDriverOptions()">
            <label for="dengan_sopir" class="cursor-pointer w-48 text-nowrap flex items-center justify-center px-6 py-3 text-center border-2 border-gray-300 rounded-md peer-checked:border-primary peer-checked:bg-primary-50 peer-checked:text-darkprimary hover:bg-gray-50 font-medium">
                Dengan Sopir
            </label>
        </div>
    </div>
    ...
    <div id="tanpa_sopir_fields" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 w-full gap-6">
        ...
        <div class="flex flex-col">
            ...
            <input type="text" class="p-2 w-full border-b border-gray-400 focus:outline-none" id="tanggal_mulai_rental" name="tanggal_mulai">
        </div>
        <div class="flex flex-col">
            <label for="tanggal_selesai_rental" class="text-sm font-bold text-primary">Tanggal Selesai</label>
            <input type="text" class="p-2 w-full border-b border-gray-400 focus:outline-none" id="tanggal_selesai_rental" name="tanggal_selesai">
        </div>
        <div class="flex flex-col">
            <label for="waktu_mulai" class="text-sm font-bold text-primary">Waktu Mulai</label>
            <input type="time" class="p-2 w-full border-b border-gray-400 focus:outline-none" id="waktu_mulai" name="waktu_mulai">
        </div>
        <div class="flex flex-col">
            <label for="waktu_selesai" class="text-sm font-bold text-primary">Waktu Selesai</label>
            <input type="time" class="p-2 w-full border-b border-gray-400 focus:outline-none" id="waktu_selesai" name="waktu_selesai">
        </div>
    </div>
    ...
    <div id="dengan_sopir_fields" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 w-full gap-6 hidden">
        ...
        <div class="flex flex-col">
            ...
            <input type="date" class="p-2 w-full border-b border-gray-400 focus:outline-none" id="tanggal_mulai_sopir" name="tanggal_mulai_sopir">
        </div>
        <div class="flex flex-col">
            <label for="durasi" class="text-sm font-bold text-primary">Durasi</label>
            <div class="flex items-center border-b border-gray-400">
                <input type="number" min="1" class="p-2 w-full focus:outline-none" id="durasi" name="durasi" placeholder="1">
                <span class="p-2 text-gray-700">Hari</span>
            </div>
        </div>
        <div class="flex flex-col">
            <label for="waktu_jemput" class="text-sm font-bold text-primary">Waktu Jemput</label>
            <input type="time" class="p-2 w-full border-b border-gray-400 focus:outline-none" id="waktu_jemput" name="waktu_mulai_sopir">
        </div>
    </div>

    <!-- Search Button -->
    <div class="flex">
        <button type="submit" class="bg-primary text-nowrap hover:bg-darkprimary text-white px-8 py-3 rounded-md uppercase font-semibold transition-all duration-300 transform hover:scale-105">
            Cari Kendaraan
        </button>
    </div>
</form>
