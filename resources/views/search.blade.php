        ...
        @if(!isset($selectedVehicle) && count($groupedVehicles) > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($groupedVehicles as $kendaraan)
                    @php
                        $totalCost = $searchParams['tipe_rental'] === 'tanpa_sopir'
                            ? $kendaraan->harga_sewa_perhari * (\Carbon\Carbon::createFromFormat('d M Y, H:i', $searchParams['start_date_formatted'])->diffInDays(\Carbon\Carbon::createFromFormat('d M Y, H:i', $searchParams['end_date_formatted'])) + 1)
