<x-user-layout title="Hasil Pencarian">
    <div class="max-w-[1600px] mx-auto p-4 md:p-8 lg:p-12">
        ...
        <section id="search" class="w-full flex items-center justify-center my-6">
            <div class="bg-white rounded-md w-full shadow-lg border-2  p-6 ">
        ...
        @if (isset($selectedVehicle) && count($relatedVehicles) > 0)
            <div class="mb-6">
                <h3 class="text-lg plus-jakarta-sans-font font-semibold text-gray-800 mb-3">Pilih Penyedia Rental Untuk: {{ $selectedVehicle }}
                </h3>
                <div class="relative">
                    ...
                    <div id="vehicle-slider" class="flex overflow-x-auto gap-4 py-2 px-8 hide-scrollbar">
                        @foreach ($relatedVehicles as $vehicle)
                            @php
                                $baseCost = $vehicle->harga_sewa_perhari * $searchParams['rentalDays'];
                                $driverCost =
                                    $searchParams['tipe_rental'] === 'dengan_sopir'
                                        ? $driver_fee * $searchParams['rentalDays']
                                        : 0;
                                $extraCost = $searchParams['extraHours'] * $searchParams['extraHourFee'];
                                $totalCost = $baseCost + $driverCost + $extraCost;
                            @endphp
                            <div
                                class="flex-shrink-0 w-64 bg-white rounded-lg shadow-md overflow-hidden relative cursor-pointer
                                {{ request()->input('vendor') == $vehicle->id_kendaraan ? 'ring-2 ring-primary' : '' }}">
                                <a href="{{ request()->fullUrlWithQuery(['selected_vehicle' => $selectedVehicle, 'vendor' => $vehicle->id_kendaraan]) }}"
                                    class="block">
                                    <div class="h-32 bg-gray-200 plus-jakarta-sans-font">
                                        @if ($vehicle->fotos)
                                    ...
                                    <div class="p-3 montserrat-font">
                                        <p class="font-medium">{{ $vehicle->nama_mitra }}</p>
                                        <p class="text-lg font-bold text-gray-900">Rp
                                            {{ number_format($totalCost, 0, ',', '.') }}</p>
                                        @if ($searchParams['extraHours'] > 0)
                                            <p class="text-xs text-gray-500">+ Ekstra {{ $searchParams['extraHours'] }}
                                                jam</p>
                                        @endif
                                    </div>
                                </a>
            ...
            @if (request()->has('vendor'))
                @php
                    ...
                    $baseCost = $selectedVendorVehicle->harga_sewa_perhari * $searchParams['rentalDays'];
                    $driverCost =
                        $searchParams['tipe_rental'] === 'dengan_sopir' ? $driver_fee * $searchParams['rentalDays'] : 0;
                    $extraCost = $searchParams['extraHours'] * $searchParams['extraHourFee'];
                    $totalCost = $baseCost + $driverCost + $extraCost;
                @endphp
                        ...
                        <div class="flex flex-col md:flex-row">
                            ...
                            <div class="md:w-1/3 h-64 md:h-auto bg-gray-200 relative">
                                <swiper-container class="mySwiper w-full h-full" pagination="true"
                                    ...
                                    @php
                                        $photos = json_decode($selectedVendorVehicle->fotos);
                                    @endphp
                            ...
                            <div class="md:w-2/3 p-6">
                                <div class="flex items-center mb-2">
                                    ...
                                    @else
                                        <div
                                            ...
                                            <span
                                                class="text-primary font-medium">{{ substr($selectedVendorVehicle->nama_mitra, 0, 1) }}</span>
                                        </div>
                                    @endif
                                    <h3 class="text-lg montserrat-font font-semibold">
                                        {{ $selectedVendorVehicle->nama_mitra }}</h3>
                                </div>

                                <h2 class="text-2xl font-bold montserrat-font text-gray-900 mb-4">
                                    {{ $selectedVendorVehicle->nama_kendaraan }}</h2>

                                <div class="flex flex-wrap gap-4 mb-4 plus-jakarta-sans-font">
                                ...
                                <div class="flex justify-between items-center mt-6 montserrat-font">
                                    <div>
                                        ...
                                        @endif
                                        @if ($searchParams['extraHours'] > 0)
                                            <p class="text-xs text-gray-500">+ Biaya ekstra
                                                {{ $searchParams['extraHours'] }} jam: Rp
                                                {{ number_format($extraCost, 0, ',', '.') }}</p>
                                        @endif
                                    </div>
                                    <a href="{{ route('detail', [
                                        'id_unit' => $selectedVendorVehicle->id_unit,
                                        ...
                                        class="bg-primary hover:bg-darkprimary text-white font-medium py-2 px-4 rounded-lg">
                                        Pesan Sekarang
                                    </a>
                                </div>
                            </div>
        ...
        @if (!isset($selectedVehicle) && count($groupedVehicles) > 0)
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach ($groupedVehicles as $kendaraan)
                    @php
                        $baseCost = $kendaraan->harga_sewa_perhari * $searchParams['rentalDays'];
                        $driverCost =
                            $searchParams['tipe_rental'] === 'dengan_sopir'
                                ? $driver_fee * $searchParams['rentalDays']
                                : 0;
                    ...
                    <div
                        ...
                        <div class="h-48 bg-gray-200 relative">
                            ...
                            <div class="absolute top-3 left-3">
                                <span
                                    class="bg-indigo-100 text-primary text-xs font-medium px-2.5 py-0.5 rounded-full">
                        ...
                        <div class="p-4">
                            <div class="flex justify-between items-start mb-2">
                                <h3 class="text-lg montserrat-font font-semibold text-gray-900">
                            ...
                            <div class="flex items-center mb-3">
                                ...
                                <span class="text-xs montserrat-font text-gray-600">{{ $kendaraan->kota }},
                                    {{ $kendaraan->kecamatan }} {{ $kendaraan->provinsi }}</span>
                            </div>
                            <div class="flex flex-wrap gap-2 mb-3">
                                <span class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">
                                    {{ $kendaraan->transmisi == 'manual' ? 'Manual' : 'Matic' }}
                                </span>
                                <span class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">
                            ...
                            <div class="flex justify-between items-center mt-4">
                                <div>
                                    <p class="text-xl montserrat-font font-bold text-gray-900">Rp
                                        {{ number_format($totalCost, 0, ',', '.') }}</p>
                                    @if ($searchParams['tipe_rental'] === 'dengan_sopir')
                                        <p class="text-xs text-gray-500">Termasuk biaya sopir Rp
                                            {{ number_format($driver_fee, 0, ',', '.') }}/hari</p>
                                ...
                                <a href="{{ request()->fullUrlWithQuery(['selected_vehicle' => $kendaraan->nama_kendaraan]) }}"
                                    class="px-4 py-2 bg-primary text-white font-medium rounded-lg hover:bg-darkprimary transition-colors">
                                    Lanjutkan
                                </a>
                            </div>
                        </div>
        ...
        @elseif(count($groupedVehicles) == 0)
            <div class="bg-white p-8 rounded-lg shadow-md text-center">
                ...
                <button onclick="history.back()"
                    class="px-4 py-2 bg-primary text-white font-medium rounded-md hover:bg-darkprimary transition-colors">
                    Ubah Pencarian
                </button>
            </div>
        @endif
