<x-mitra-layout title="Edit Rekening Mitra">
    <div class="p-4 w-full h-full flex items-center justify-center">
        <form action="{{ route('mitra.rekeningMitra.RekeningUpdate', $rekeningMitra->id_rekening_mitra) }}" method="POST"
            ...
            <div class="col-span-2 flex flex-col gap-2">
                ...
                <select name="id_metode_pembayaran_mitra"
                    ...
                    @foreach ($metodePembayaranMitra as $item)
                        <option
                            {{ $item->id_metode_pembayaran_mitra == $rekeningMitra->id_metode_pembayaran_mitra ? 'selected' : '' }}
                            value="{{ $item->id_metode_pembayaran_mitra }}">{{ $item->nama_metode }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex flex-col gap-2">
                <label for="nomor_rekening">Nomor Rekening</label>
                <input type="text" id="nomor_rekening" value="{{ $rekeningMitra->nomor_rekening }}"
                    name="nomor_rekening" class="focus:outline-none border border-primary rounded-md p-2">
            </div>
            <div class="flex flex-col gap-2">
                <label for="nama_pemilik">Nama Pemilik</label>
                <input type="text" id="nama_pemilik" value="{{ $rekeningMitra->nama_pemilik }}" name="nama_pemilik"
