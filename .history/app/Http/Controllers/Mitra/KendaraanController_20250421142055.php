...
public function storePengembalian(Request $request)
{
    ...
    try {
        ...
        if ($existing) {
            $existing->delete(); // Hapus lama
            // lanjut untuk menyimpan yang baru
        }

        // Simpan file jika ada
        $fotoSebelum = $request->hasFile('foto_sebelum')
            ? $request->file('foto_sebelum')->store('foto_pengembalian')
            : null;

        $fotoSesudah = $request->hasFile('foto_sesudah')
            ? $request->file('foto_sesudah')->store('foto_pengembalian')
            : null;
