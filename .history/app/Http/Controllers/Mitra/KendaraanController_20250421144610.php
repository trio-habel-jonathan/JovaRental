...
class KendaraanController extends Controller
{
    ...
    public function storePengembalian(Request $request, $id_unit)
    {
        ...
        if ($existing) {
            // Debugging: Show that we found an existing pengembalian
            dd('Existing Pengembalian found:', $existing);
    
            $existing->delete(); // Hapus data lama
            return redirect()->route('mitra.pengembalian.create', [$id_unit])
                             ->with('success', 'Data pengembalian lama berhasil dihapus.');
        }
        ...
        // Debugging: Show if we found the detailPemesanan
        dd('Detail Pemesanan:', $detailPemesanan);
    
        if (!$detailPemesanan) {
            return redirect()->route('mitra.pengembalian.create', [$id_unit])
                             ->with('error', 'Detail pemesanan tidak ditemukan.')
                             ->withInput();
        }
        ...
        // Debugging: Show id_detail value
        dd('ID Detail:', $id_detail);
        ...
        // Debugging: Show file data
        dd('Foto Sebelum:', $fotoSebelum, 'Foto Sesudah:', $fotoSesudah);
        ...
        // Debugging: Show the data before saving
        dd('Pengembalian Data:', $pengembalian);
